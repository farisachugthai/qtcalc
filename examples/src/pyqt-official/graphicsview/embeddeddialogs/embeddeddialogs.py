...
from PyQt5.QtGui import QBrush, QColor, QPainter, QPainterPath, QPixmap, QTransform
...
class CustomProxy(QGraphicsProxyWidget):
    ...
    def sceneEventFilter(self, watched, event):
        if watched.isWindow() and (
            event.type() == QEvent.UngrabMouse or event.type() == QEvent.GrabMouse
        ):
...
class EmbeddedDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.ui.layoutDirection.setCurrentIndex(
            self.layoutDirection() != Qt.LeftToRight
        )
...
if __name__ == "__main__":
    ...
    view.scale(0.5, 0.5)
    view.setRenderHints(
        view.renderHints() | QPainter.Antialiasing | QPainter.SmoothPixmapTransform
    )
