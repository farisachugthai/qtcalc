...
class MenuManager(QObject):
    ...
    def itemSelected(self, userCode, menuName):
        if userCode == MenuManager.LAUNCH:
            ...
            # Out.
            self.score.queueMovie(
                self.currentMenu + " -out", Score.FROM_START, Score.LOCK_ITEMS
            )
            self.score.queueMovie(
                self.currentMenuButtons + " -out", Score.FROM_START, Score.LOCK_ITEMS
            )
            self.score.queueMovie(self.currentInfo + " -out")
            self.score.queueMovie(
                self.currentInfo + " -buttons -out", Score.NEW_ANIMATION_ONLY
            )
            ...
            self.score.queueMovie(
                self.currentMenu, Score.FROM_START, Score.UNLOCK_ITEMS
            )
            self.score.queueMovie(
                self.currentMenuButtons, Score.FROM_START, Score.UNLOCK_ITEMS
            )
            ...
            self.score.queueMovie(
                self.currentInfo + " -buttons", Score.NEW_ANIMATION_ONLY
            )
            ...
            if backMenu:
                self.score.queueMovie(
                    self.currentMenu + " -top_out", Score.FROM_START, Score.LOCK_ITEMS
                )
                self.score.queueMovie(
                    backMenu + " -bottom_in", Score.FROM_START, Score.UNLOCK_ITEMS
                )
            ...
            if moreMenu:
                self.score.queueMovie(
                    self.currentMenu + " -bottom_out",
                    Score.FROM_START,
                    Score.LOCK_ITEMS,
                )
                self.score.queueMovie(
                    moreMenu + " -top_in", Score.FROM_START, Score.UNLOCK_ITEMS
                )
            ...
            if self.currentMenuCode == MenuManager.MENU2:
                # Out.
                self.score.queueMovie(
                    self.currentInfo + " -out", Score.NEW_ANIMATION_ONLY
                )
                self.score.queueMovie(
                    self.currentInfo + " -buttons -out", Score.NEW_ANIMATION_ONLY
                )
    ...
    def readInfoAboutExample(self, example):
        ...
        if name in self.info:
            Colors.debug(
                "__WARNING: MenuManager.readInfoAboutExample: "
                "Demo/example with name",
                name,
    ...
    def createRootMenu(self, el):
        ...
        self.createLowLeftButton(
            "Quit", MenuManager.QUIT, menuButtonsIn, menuButtonsOut, None
        )
        self.createLowRightButton(
            "Toggle fullscreen",
            MenuManager.FULLSCREEN,
            menuButtonsIn,
            menuButtonsOut,
            None,
        )
    ...
    def createLeafMenu(self, el):
        ...
        if el.getAttribute("executable") != "false":
            self.createLowRightLeafButton(
                "Launch", 405, MenuManager.LAUNCH, infoButtonsIn, infoButtonsOut, None
            )
    ...
    def createMenu(self, category, type):
        ...
        while currentNode is not None:
            ...
            while currentNode is not None and i < maxExamples:
                ...
                anim.setKeyValueAt(
                    0.60, QPointF(xOffset, (i * ihp) + yOffset + Colors.contentStartY)
                )
                anim.setKeyValueAt(
                    0.70,
                    QPointF(
                ...
                anim.setKeyValueAt(
                    0.80, QPointF(xOffset, (i * ihp) + yOffset + Colors.contentStartY)
                )
                anim.setKeyValueAt(
                    0.90,
                    QPointF(
                ...
                anim.setEndValue(
                    QPointF(xOffset, (i * ihp) + yOffset + Colors.contentStartY)
                )
                movieIn.append(anim)
                ...
                anim.setKeyValueAt(
                    0.55,
                    QPointF(
                        xOffset, (i * ihp) + yOffset + Colors.contentStartY - i * 2.0
                    ),
                )
                anim.setKeyValueAt(
                    0.70,
                ...
                anim.setKeyValueAt(
                    0.80,
                    QPointF(
                        xOffset, (i * ihp) + yOffset + Colors.contentStartY - i * 1.0
                    ),
                )
                anim.setKeyValueAt(
                    0.90,
                ...
                anim.setKeyValueAt(
                    0.70, QPointF(xOffset, yOffset + Colors.contentStartY)
                )
                ...
                anim.setKeyValueAt(
                    0.70,
                    QPointF(
                        xOffset, (maxExamples * ihp) + yOffset + Colors.contentStartY
                    ),
                )
                anim.setEndValue(
                    QPointF(-iw, (maxExamples * ihp) + yOffset + Colors.contentStartY)
                )
    ...
    def createLowLeftButton(
        self, label, type, movieIn, movieOut, movieShake, menuString=""
    ):
        button = TextButton(
            label, TextButton.RIGHT, type, self.window.mainSceneRoot, TextButton.PANEL
        )
        if menuString:
        ...
        buttonIn.setKeyValueAt(
            0.5, QPointF(-iw, Colors.contentStartY + Colors.contentHeight - 35)
        )
        buttonIn.setKeyValueAt(
            0.7, QPointF(xOffset, Colors.contentStartY + Colors.contentHeight - 35)
        )
    ...
    def createLowRightButton(self, label, type, movieIn, movieOut, movieShake):
        ...
        anim.setKeyValueAt(
            0.7,
            QPointF(xOffset + 535, Colors.contentStartY + Colors.contentHeight - 35),
        )
        anim.setEndValue(
            QPointF(xOffset + 535, Colors.contentStartY + Colors.contentHeight - 26)
        )
        movieIn.append(anim)
    ...
    def createLowRightLeafButton(
        self, label, xOffset, type, movieIn, movieOut, movieShake
    ):
        item = TextButton(
            label, TextButton.RIGHT, type, self.window.mainSceneRoot, TextButton.PANEL
        )
        ...
        anim.setKeyValueAt(
            0.10, QPointF(sw, Colors.contentStartY + Colors.contentHeight - 35)
        )
        anim.setKeyValueAt(
            0.30, QPointF(xOffset, Colors.contentStartY + Colors.contentHeight - 35)
        )
        anim.setKeyValueAt(
            0.35,
            QPointF(xOffset + 30, Colors.contentStartY + Colors.contentHeight - 35),
        )
        anim.setKeyValueAt(
            0.40, QPointF(xOffset, Colors.contentStartY + Colors.contentHeight - 35)
        )
        anim.setKeyValueAt(
            0.45, QPointF(xOffset + 5, Colors.contentStartY + Colors.contentHeight - 35)
        )
        anim.setKeyValueAt(
            0.50, QPointF(xOffset, Colors.contentStartY + Colors.contentHeight - 35)
        )
        anim.setEndValue(
            QPointF(xOffset, Colors.contentStartY + Colors.contentHeight - 26)
        )
    ...
    def createInfo(self, item, name):
        ...
        infoIn.setStartValue(
            QPointF(self.window.scene.sceneRect().width(), Colors.contentStartY)
        )
    ...
    def createTicker(self):
        ...
        self.tickerInAnim.setStartValue(
            QPointF(
                self.window.scene.sceneRect().width(), Colors.contentStartY + qtPosY
            )
        )
        ...
        qtOut.setEndValue(
            QPointF(
                self.window.scene.sceneRect().width() + 700,
    ...
    def createUpnDownButtons(self):
        ...
        shakeAnim = DemoItemAnimation(
            self.downButton, DemoItemAnimation.ANIM_UNSPECIFIED
        )
        shakeAnim.setDuration(650)
    ...
    def createBackButton(self):
        ...
        self.createLowLeftButton(
            "Back", MenuManager.ROOT, backIn, backOut, backShake, Colors.rootMenuName
        )
